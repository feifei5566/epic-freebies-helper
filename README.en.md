...
The project is built on top of community open-source work and supports the official `Gemini` API, `AiHubMix`-compatible access, and `GLM` multimodal models. Its core objective is to keep auto-login, captcha recognition, and checkout stable. If you already have a Google AI Studio Gemini key, you usually do not need to touch the code at all.
...
- A Google AI Studio Gemini key, or a compatible service key, prepared for captcha solving. GLM (`GLM_API_KEY`) remains supported if you prefer it.
